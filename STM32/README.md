...
- Pull-up resistors for I2C communication, typically 10kΩ and often included on modules
...
- Applies a **single-state EMA filter** to smooth noise with less latency than a moving average
- Reduces false detections from sensor noise while keeping the signal responsive
- Formula: `filtered_value = (alpha * new_sample) + ((1 - alpha) * previous_filtered)`
- Current firmware starts with `alpha = 0.3`
...
| ---------------- | --------------- | ------------------------------------------------------------ |
| STAND_THRESHOLD  | boot-calibrated | Acceleration threshold for standing position                 |
| DOWN_THRESHOLD   | boot-calibrated | Threshold to trigger squat descent                           |
| BOTTOM_THRESHOLD | boot-calibrated | Minimum acceleration (bottom of squat)                       |
| UP_THRESHOLD     | boot-calibrated | Threshold to start ascent detection                          |
| REP_LOCKOUT_MS   | 1100            | Minimum time between squat counts (prevents double-counting) |
| MIN_STATE_HOLD   | 2               | Consecutive reads required before a state transition         |
...
The threshold values are now **derived from a boot-time rest calibration** and still may need tuning for your sensor placement and body. The algorithm assumes:
...
2. Keep still during startup so the 20-sample calibration can capture a clean standing baseline
3. Open a serial monitor (115200 baud) to view output
4. Perform some squats and observe the RAW_AXIS and FILTERED values
5. If counts are still too loose or too strict, adjust the calibration offsets in `Core/Src/main.c`
6. The current firmware uses tighter squat gating to reduce false counts:
   - Longer rep lockout
   - Minimum 2-sample state hold
   - ROM check before incrementing the rep count
...
Extract Selected Axis (AY)
      ↓
Boot Calibration (rest baseline -> thresholds)
      ↓
Squat State Machine
      ├→ Check current state
      ├→ Evaluate thresholds
...
| File                            | Purpose                                                                                      |
| ------------------------------- | -------------------------------------------------------------------------------------------- |
| `Core/Src/main.c`               | Main application logic (squat detection, EMA filtering, calibration, I2C/UART communication) |
...
Implements a lightweight EMA filter to reduce noise without a sample buffer

### Calibrate_Squat_Thresholds()

Samples the IMU at rest during startup and derives squat thresholds from the measured baseline

### Squat_Update()

Main state machine function that processes the filtered acceleration value, applies state-hold validation, and tracks squat progression
...
- ✅ Boot-time threshold calibration
